<html> 
	<head> 
		<title>welcome jboot!</title> 
	</head> 
	<body> 
		<ul>
			<li><a href="static.txt">static.txt</a></li>
			<li><a href="resources.txt">resources.txt</a></li>
			<li><a href="public.txt">public.txt</a></li>
			<li><a href="meta-inf.resources.txt">meta-inf.resources.txt</a></li>
			<li><a href="contact">contact(json)</a></li>
			<li><a href="contact.ftl">contact(freemarker)</a></li>
			<li><a href="error404">404</a></li>
			<li><a href="error500">500</a></li>
		</ul>
	</body> 
</html> 